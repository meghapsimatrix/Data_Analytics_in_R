...
-   `stringr` is a package in `tidyverse ~` manipulate and work with strings
...
-   `forcats` is a package in `tidyverse` \~ work with factors
...
levels(cdc_cat$new_cat)
```

## `fct_infreq()`

```{r}
ggplot(cdc_cat, aes(x = fct_infreq(category), y = n)) +
  geom_bar(stat = "identity") +
  theme_bw()
```
...
Here, we are using the full `cdc_data`:
